<template>
    <div>
        ...
        <div class="row">
            <div class="col-md-3">
                <input type="text" class="form-control" style="height: auto !important;"
                    :name="'name_' + ticket_index + '_' + quantity_index"
                    v-model="$parent.name[ticket_index][quantity_index]" v-validate="'required'"
                    :placeholder="trans('em.name') + '*'">
            </div>

            <div class="col-md-3">
                <input type="text" class="form-control" style="height: auto !important;"
                    :name="'phone_' + ticket_index + '_' + quantity_index"
                    v-model="$parent.phone[ticket_index][quantity_index]" v-validate="'required'"
                    :placeholder="trans('em.phone') + '*'">
            </div>

            <div class="col-md-6">
                <textarea class="form-control" rows="1" style="height: auto !important;"
                    :name="'address_' + ticket_index + '_' + quantity_index"
                    v-model="$parent.address[ticket_index][quantity_index]" v-validate="'required'"
                    :placeholder="trans('em.address') + '*'"></textarea>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
